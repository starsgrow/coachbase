...
export async function POST(request) {
  try {
    ...
    const [globalesRes, coachRes] = await Promise.all([
      supabaseServer.from("tabla_ejercicios").select("slug, nombre_ejercicio, musculos_principales, equipamiento_necesario"),
      supabaseServer.from("coach_ejercicios").select("slug, nombre, grupo_muscular, equipamiento").eq("coach_id", coach_id)
    ]);
...
${JSON.stringify(listaEjercicios.slice(0, 100), null, 2)}
...
Devuelve ÚNICAMENTE el array JSON puro.`;
      ...
      // Limpiar prefijos [DEL COACH] en los nombres devueltos si la IA los copió
...
${JSON.stringify(listaEjercicios.slice(0, 120), null, 2)}
